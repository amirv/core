...
class TadiranClimate(ClimateEntity):
    ...
    def __init__(self, name, ip, rm_type):
        """Initialize a Tadiran climate device."""
        self._name = name
        self._ip = ip
        self._id = slugify(ip)
        self._state = False
    ...
    def max_temp(self) -> float:
        """Return maximal supported temperature."""
...
class BroadlinkTadiran:
    """Represents a Broadlink controlled Tadiran AC."""
    ...
    @property
    def available(self):
        """Return true if there is connectivity to device."""
        return self._available

    def send(self, args_vars):
        """Send parameters to AC through Broadlink."""
        data = {}
    ...
    def is_alive(self):
        """Return true if broadlink device is connected and alive."""
